function vl_plotsiftdesc_mcho2(d,f,varargin)
...
for k=1:K
  ...
  [wg, xg, yg, xn, yn, xb, yb] = render_descr2(d(:,k), NBP, NBO, maxv, minv, bContrastInsenstive) ;
...
switch drawingtype
    case 'gradient'
        drawWeightedline(wg_all, xg_all, yg_all);
    case 'edge'
        drawWeightedline(wg_all, xe_all, ye_all);
    otherwise
end
...
% --------------------------------------------------------------------
function drawWeightedline(w, lineX, lineY )
% --------------------------------------------------------------------

% positive weight edges

[ wval widx ] = sort(w,'ascend');
...
for i = 1:numel(widx)
    ...
    plot( lineX(:, widx(i)) , lineY(:, widx(i)), ...
        '-', 'Color', colorCode, 'LineWidth', 2);
    %pause;
end        
...
% --------------------------------------------------------------------
function [wg, xg, yg, xe, ye, xb,yb] = render_descr2(d, BP, BO, maxv, minv, bContrastInsenstive)
% --------------------------------------------------------------------
...
xde = repmat(cos(th+pi/2), 1, BP*BP)*0.5 ;
yde = repmat(sin(th+pi/2), 1, BP*BP)*0.5 ;
